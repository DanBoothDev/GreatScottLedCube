...
int stretch=100;             // set timer for the change of the shown picture, if animation should move really fast then low stretch value, if animation should move really slow than big stretch value
...
void loop(){
  ...
  if(micros()>= oldMicros){                                           // waits until LayerDuration is reached and than goes through cycle
   ...
   bright=analogRead(Potentiometer);                                  // use analogRead to set value for int bright from 0-1023
